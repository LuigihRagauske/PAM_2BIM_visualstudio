...
namespace AppCopaHAS.ViewModels.Jogos
{
    ...
    public JogoViewModel()                                         
    {
        ...
        Jogos = new ObservableCollection<Jogo>();
            _ = ObterEstadios();
            _ = ObterSelecoes();
        // O "_" descarta a operação assíncrona sem travar o construtor

            SalvarCommand = new ICommand(async () => { await SalvarResultado(); });
    }
        ...
        {
            get => _dataSelecionada.Date + _horaSelecionada;
        }
        ...
        public async Task SalvarResultado()
        {
            try
            {
                ...
                await AppShell.Current.GoToAsync("//tabela");
            }
            catch (Exception ex)
            {
                if (Application.Current?.MainPage != null)
        ...
        public async Task ObterSelecoes()
        {
            ...
            {
                // Captará o erro para exibir em tela com segurança para o .NET 10
                if (Application.Current?.MainPage != null)
                {
